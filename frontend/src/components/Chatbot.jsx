...
export default function Chatbot() {
  ...
  const send = async () => {
    ...
    try {
      ...
      setMessages((m) => [...m, { role: 'ai', text: message || 'Okay.' }])
      if (appointment) {
        setToast('Appointment created!')
      }
    } catch (e) {
      setMessages((m) => [...m, { role: 'ai', text: "Sorry, I couldn't process that." }])
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="max-w-3xl mx-auto p-4">
      ...
      <div className="mt-3 flex gap-2">
        <textarea
          ...
          placeholder="e.g., Book me with Dr. Kim tomorrow at 10am for checkup"
          className="flex-1 border rounded p-3 focus:outline-none focus:ring"
          rows={2}
        />
        <button onClick={send} className="px-4 py-2 rounded bg-blue-600 text-white disabled:opacity-60" disabled={loading}>
          Send
        </button>
      </div>
